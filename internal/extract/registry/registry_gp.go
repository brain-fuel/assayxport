...
func RunOutcome(exts []extract.Extractor, root string) extract.ExtractionOutcome {
	...
	if len(pkgs) == 0 && len(failures) > 0 {
		return extract.ExtractionFailed{Failures: failures}
	}
	for l := range langSet {
		languages = append(languages, l)
	}
	sort.Strings(languages)
	if len(failures) > 0 {
		return extract.ExtractionPartial{Packages: pkgs, Languages: languages, Warnings: failures}
	}
	return extract.ExtractionComplete{Packages: pkgs, Languages: languages}
}

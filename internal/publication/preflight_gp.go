// Code generated by goplus from preflight.gp. DO NOT EDIT.
//goplus:v v0.28.0
...
func Preflight(root string) (Config, Report) {
	...
	for _, issue := range issues {
		if issue.Code == "CONFIG_MISSING" {
			return cfg, Report{Issues: []Issue{issue}}
		}
	}
...
func loadConfig(path string) (Config, []Issue) {
	...
	for lineNo := 1; s.Scan(); lineNo++ {
		...
		if !ok {
			issues = append(issues, Issue{"CONFIG_SYNTAX", fmt.Sprintf("%s:%d is not key = value", path, lineNo), "Write the setting as a TOML key followed by = and a quoted string value."})
			continue
		}
		qualified := section + "." + key
		if seen[qualified] {
			issues = append(issues, Issue{"CONFIG_DUPLICATE", fmt.Sprintf("%s:%d repeats %s", path, lineNo, qualified), "Remove the duplicate setting so publication metadata has one unambiguous value."})
			continue
		}
		seen[qualified] = true
		if section == "" && key == "schema_version" {
			...
			if e != nil {
				issues = append(issues, Issue{"CONFIG_SCHEMA_INVALID", "schema_version is not an integer", "Set schema_version = 1."})
			} else {
				schema = n
			}
			continue
		}
		value, e := strconv.Unquote(raw)
		if e != nil {
	...
	if err := s.Err(); err != nil {
		issues = append(issues, Issue{"CONFIG_READ_FAILED", err.Error(), "Make assayxport.toml readable and rerun the preflight."})
	}
	if schema != 1 {
...
func set(c *Config, key, value string) bool {
	...
	case "go.module":
		c.GoModule = value
	case "go.repository":
		c.GoRepository = value
	case "maven.group_id":
		c.GroupID = value
	case "maven.artifact_id":
		c.ArtifactID = value
	case "maven.build_manifest":
		c.BuildManifest = value
	case "maven.name":
		c.Name = value
	case "maven.description":
		c.Description = value
	case "maven.url":
		c.URL = value
	case "maven.license_name":
		c.LicenseName = value
	case "maven.license_url":
		c.LicenseURL = value
	case "maven.developer_id":
		c.DeveloperID = value
	case "maven.developer_name":
		c.DeveloperName = value
	case "maven.developer_email":
		c.DeveloperEmail = value
	case "maven.developer_url":
		c.DeveloperURL = value
	case "maven.scm_url":
		c.SCMURL = value
	case "maven.scm_connection":
		c.SCMConnection = value
	case "maven.scm_developer_connection":
		c.SCMDeveloperConnection = value
	case "maven.bundle":
		c.Bundle = value
	default:
		return false
...
func validateConfig(c Config) []Issue {
	...
	for _, item := range required {
		if strings.TrimSpace(item.value) == "" {
			issues = append(issues, Issue{"CONFIG_REQUIRED_MISSING", "required setting " + item.name + " is missing or empty", "Add " + item.name + " to assayxport.toml with the authoritative publication value."})
		}
	}
	if c.Policy != "" && c.Policy != "goplus-dual" && c.Policy != "java-library" {
		issues = append(issues, Issue{"POLICY_UNSUPPORTED", "policy " + strconv.Quote(c.Policy) + " is unsupported", "Set policy = \"goplus-dual\" for Go+ dual publication or \"java-library\" for an ordinary Java library."})
	}
	if c.Version != "" && !placeholder(c.Version) && !regexp.MustCompile(`^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$`).MatchString(c.Version) {
		issues = append(issues, Issue{"VERSION_INVALID", "version must be a release X.Y.Z without a v prefix", "Set version to the Maven version, for example \"0.4.0\"; the Go version is derived as v0.4.0."})
	}
	if c.GroupID != "" && !regexp.MustCompile(`^[A-Za-z0-9_]+(\.[A-Za-z0-9_]+)+$`).MatchString(c.GroupID) {
		issues = append(issues, Issue{"MAVEN_GROUP_INVALID", "maven.group_id is not a reverse-domain Maven group", "Use a controlled reverse-domain group such as \"dev.goforge\"."})
	}
	if c.ArtifactID != "" && !regexp.MustCompile(`^[a-z0-9][a-z0-9._-]*$`).MatchString(c.ArtifactID) {
		issues = append(issues, Issue{"MAVEN_ARTIFACT_INVALID", "maven.artifact_id contains unsupported characters", "Use a lowercase Maven artifact id containing letters, digits, dots, underscores, or hyphens."})
	}
	if c.DeveloperEmail != "" && !placeholder(c.DeveloperEmail) {
		if _, err := mail.ParseAddress(c.DeveloperEmail); err != nil {
			issues = append(issues, Issue{"DEVELOPER_EMAIL_INVALID", "maven.developer_email is not a valid address", "Set developer_email to a monitored publisher contact address."})
		}
	}
	for _, item := range []struct{ name, value string }{{"go.repository", c.GoRepository}, {"maven.url", c.URL}, {"maven.license_url", c.LicenseURL}, {"maven.developer_url", c.DeveloperURL}, {"maven.scm_url", c.SCMURL}} {
		if item.value != "" && !placeholder(item.value) {
			parsed, err := url.Parse(item.value)
			if err != nil || parsed.Scheme != "https" || parsed.Host == "" {
				issues = append(issues, Issue{"URL_INVALID", item.name + " must be an absolute HTTPS URL", "Set " + item.name + " to its canonical https:// URL."})
			}
		}
	}
	if c.Bundle != "" {
		if _, ok := safePath(".", c.Bundle); !ok {
			issues = append(issues, Issue{"BUNDLE_PATH_INVALID", "maven.bundle escapes the project root", "Use a relative path such as dist/central/name-version-bundle.zip."})
		}
	}
	for _, item := range required {
		if strings.Contains(strings.ToUpper(item.value), "TODO") {
...
func Init(root string) (string, error) {
	path := filepath.Join(root, "assayxport.toml")
	if _, err := os.Stat(path); err == nil {
		return "", fmt.Errorf("[CONFIG_EXISTS] %s already exists\n  Fix: edit the existing file, or move it aside intentionally before rerunning `ax publish --init`", path)
	} else if !os.IsNotExist(err) {
		return "", err
	}
	module := readModule(filepath.Join(root, "go.mod"))
	if module == "" {
		module = "TODO_GO_MODULE"
	}
	artifact := filepath.Base(module)
	if artifact == "." || artifact == "/" || strings.Contains(artifact, "TODO") {
		artifact = "TODO_ARTIFACT_ID"
	}
	repository := gitOutput(root, "remote", "get-url", "origin")
	httpsRepo := repositoryHTTPS(repository)
	if httpsRepo == "" {
		httpsRepo = "TODO_HTTPS_REPOSITORY"
	}
	developer := repositoryOwner(httpsRepo)
	if developer == "" {
		developer = "TODO_DEVELOPER_ID"
	}
	version := "TODO_VERSION"
	name := strings.ToUpper(artifact[:1]) + artifact[1:]
	page := "TODO_PROJECT_URL"
	if strings.HasPrefix(module, "goforge.dev/") {
		page = "https://goforge.dev/" + artifact + "/"
	}
...
func repositoryHTTPS(raw string) string {
...
func repositoryOwner(raw string) string {
...
func validateBuild(root string, c Config) []Issue {
	...
	path, ok := safePath(root, c.BuildManifest)
	if !ok {
		return []Issue{{"BUILD_MANIFEST_PATH_INVALID", "maven.build_manifest escapes the project root", "Use a relative path inside the checkout, normally .goplus/build/java/publication.json."}}
	}
	data, err := os.ReadFile(path)
	if err != nil {
		return []Issue{{"BUILD_MANIFEST_MISSING", fmt.Sprintf("cannot read build manifest %s: %v", c.BuildManifest, err), "Run `go tool goplus build --target java ./...` from the clean tagged checkout, then rerun `ax publish --prepare`."}}
	}
	var m buildManifest
	if err := json.Unmarshal(data, &m); err != nil {
		return []Issue{{"BUILD_MANIFEST_INVALID", err.Error(), "Regenerate it with `go tool goplus build --target java ./...`; do not hand-edit publication.json."}}
	}
	issues := []Issue{}
	...
	for _, out := range m.Outputs {
		artifact, safe := safePath(root, out.Path)
		if !safe {
			issues = append(issues, Issue{"BUILD_OUTPUT_PATH_INVALID", "manifest output escapes project root: " + out.Path, "Regenerate the manifest; output paths must be relative and remain inside the checkout."})
			continue
		}
		bytes, e := os.ReadFile(artifact)
		if e != nil {
			issues = append(issues, Issue{"BUILD_OUTPUT_MISSING", "manifest output is missing: " + out.Path, "Run `go tool goplus build --target java ./...` and do not move artifacts before publishing."})
			continue
		}
...
func validateGitRelease(root string, c Config) []Issue {
	...
	issues := []Issue{}
	if releaseCheckoutDirty(root, c.Version) {
		issues = append(issues, Issue{"GIT_CHECKOUT_DIRTY", "release checkout has uncommitted or untracked files other than ax's resumable release state", "Commit the intended release files and remove or ignore generated scratch files; `git status --short` may contain only .assayxport/releases/" + c.Version + ".json before retrying."})
	}
...
func releaseCheckoutDirty(root, version string) bool {
	allowed := "?? .assayxport/releases/" + version + ".json"
	status := gitOutput(root, "status", "--porcelain=v1", "--untracked-files=all")
	for _, line := range strings.Split(status, "\n") {
		if strings.TrimSpace(line) != "" && line != allowed {
			return true
		}
	}
	return false

...
func Publish(ctx context.Context,root string,cfg Config,prepared Prepared,settingsPath string,progress Progress)(Deployment,error){
	report:=func(message string){if progress!=nil{progress(message)}}
	...
	if id==""{if len(prepared.PublicKey)>0{report("publishing the OpenPGP public key");if err=publishPublicKey(ctx,prepared.PublicKey);err!=nil{return Deployment{},err}};report("uploading the Central bundle");id,err=client.Upload(ctx,prepared.Bundle,cfg.GroupID+":"+cfg.ArtifactID+":"+cfg.Version);if err!=nil{return Deployment{},err};state=releaseState{cfg.Version,digest,id,"UPLOADED"};if err=writeReleaseState(statePath,state);err!=nil{return Deployment{},err};report("Central deployment "+id+" uploaded")}else{report("resuming Central deployment "+id+" from saved state "+state.State)}
	report("waiting for Central validation")
	deployment,err:=client.Wait(ctx,id,"VALIDATED",progress);if err!=nil{return deployment,err};state.State=deployment.DeploymentState;_ = writeReleaseState(statePath,state)
	if deployment.DeploymentState=="VALIDATED"{report("requesting Central promotion");if err=client.Promote(ctx,id);err!=nil{return deployment,err};report("Central promotion accepted")}else{report("Central is already "+deployment.DeploymentState+"; promotion does not need to be repeated")}
	report("waiting for publication to Maven Central")
	deployment,err=client.Wait(ctx,id,"PUBLISHED",progress);if err!=nil{return deployment,err};state.State=deployment.DeploymentState;if err=writeReleaseState(statePath,state);err!=nil{return deployment,err};report("Central deployment is PUBLISHED");return deployment,nil
}
...
func(c CentralClient)Wait(ctx context.Context,id,target string,progress Progress)(Deployment,error){last:="";for{d,err:=c.Status(ctx,id);if err!=nil{return d,err};if d.DeploymentState!=last{if progress!=nil{progress("Central state: "+d.DeploymentState)};last=d.DeploymentState};if d.DeploymentState=="FAILED"{return d,fmt.Errorf("Central deployment failed: %v",d.Errors)};if stateRank(d.DeploymentState)>=stateRank(target){return d,nil};select{case<-ctx.Done():return d,ctx.Err();case<-time.After(2*time.Second):}}}
func stateRank(state string)int{switch state{case "PENDING":return 1;case "VALIDATING":return 2;case "VALIDATED":return 3;case "PUBLISHING":return 4;case "PUBLISHED":return 5};return 0}

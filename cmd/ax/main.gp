...
func runPublishCmd(args []string) error {
	...
	mode := "publication"
	if *prepare { mode = "bundle preparation" }
...
func runAssayCmd(args []string) error {
	...
	if strings.HasPrefix(path, "mvn:") && *documentation != "off" {
		...
		for _, classifier := range []string{"sources", "javadoc"} {
			candidate := coord; candidate.Classifier = classifier
			resolved, resolveErr := resolver.Resolve(context.Background(), candidate)
			if resolveErr != nil {
				if *documentation == "require" { return fmt.Errorf("documentation classifier %s: %w", classifier, resolveErr) }
				fmt.Fprintf(os.Stderr, "ax: documentation %s: missing (%v)\n", classifier, resolveErr)
				continue
			}
			var assessment doccheck.Assessment
			if classifier == "sources" { assessment, err = doccheck.InspectSources(resolved.Path) } else { assessment, err = doccheck.InspectJavadoc(resolved.Path) }
			if err != nil { return err }
			fmt.Fprintf(os.Stderr, "ax: documentation %s: %s\n", classifier, assessment.Status)
			if *documentation == "require" && assessment.Status != doccheck.Complete { return fmt.Errorf("documentation classifier %s is %s", classifier, assessment.Status) }
		}
	}
	if isArtifact {
